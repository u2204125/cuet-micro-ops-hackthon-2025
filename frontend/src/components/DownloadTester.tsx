...
import { Download, Loader2, CheckCircle, XCircle, Clock, Zap } from 'lucide-react';
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            ...
            <p className="text-gray-600 mb-4 text-sm">
                Initiates a download job and polls for status. Watch progress in real-time as the queue processes files!
            </p>
            ...
            >
                {isActive ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
            ...
            {jobStatus && (
                <div className="mt-4 space-y-3">
                    ...
                    {/* Results */}
                    {jobStatus.result && (
                        <div className={`p-4 rounded border ${isComplete ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                            <p className="text-sm font-medium mb-2">{jobStatus.result.message}</p>
                            <div className="text-xs text-gray-700 space-y-1">
                                <div>✓ Success: {jobStatus.result.successCount}/{jobStatus.totalFiles}</div>
                                <div>✗ Failed: {jobStatus.result.failedCount}/{jobStatus.totalFiles}</div>
                                {jobStatus.result.downloadUrls && jobStatus.result.downloadUrls.length > 0 && (
