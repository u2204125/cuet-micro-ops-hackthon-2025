    ...
    const triggerBackendError = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                window.dispatchEvent(new CustomEvent('api-call', { detail: { type: 'error' } }));
            }
        } catch (error) {
            const errorLog: ErrorLog = {
                id: crypto.randomUUID(),
                timestamp: new Date().toISOString(),
                message: error instanceof Error ? error.message : 'Network error - Failed to reach backend',
            ...
            <div className="mb-4 p-3 bg-blue-50 rounded border border-blue-100 text-sm">
                <div className="flex items-center justify-between">
                    <span className="text-blue-800">
                        <strong>Sentry Integration:</strong> {sentryConfigured ? '✅ Active' : '⚠️ Demo Mode'}
                    </span>
                    {sentryConfigured && (
                        <a
                ...
                {!sentryConfigured && (
                    <p className="text-xs text-blue-600 mt-1">
                        Errors captured locally for demonstration. Set VITE_SENTRY_DSN to enable cloud tracking.
                    </p>
                )}
            </div>
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className={`text-xs font-medium px-2 py-0.5 rounded ${error.type === 'frontend'
